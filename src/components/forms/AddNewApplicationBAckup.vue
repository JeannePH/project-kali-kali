...
const handleFileUpload = (event) => {
  ...
  for (const file of files) {
    if (file && file.type === "application/json") {
      ...
      console.error(`${file ? file.name : 'Le fichier'} n'est pas un fichier JSON valide.`);
    }
  }
};
...
const addApplication = async () => {
  try {
    ...
    if (!name.value || filesContent.value.length === 0) {
      console.error("Il faut remplir le nom de l'application et télécharger un fichier JSON.");
      return;
    }
    await processFiles(filesContent.value, name.value);
    resetForm();
  } catch (error) {
